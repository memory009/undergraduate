...
def main():
    ...
    try:
        ...
        output_path_picture = '../outputs/picture'
        if not os.path.exists(output_path_picture):
            os.makedirs(output_path_picture)

        output_path_pointclouds = '../outputs/pointclouds'
        if not os.path.exists(output_path_pointclouds):
            os.makedirs(output_path_pointclouds)

        # set the callback function
        camera.listen(lambda image: image.save_to_disk(os.path.join(output_path_picture, '%06d.png' % image.frame)))
        ...
        lidar.listen(
            lambda point_cloud: point_cloud.save_to_disk(os.path.join(output_path_pointclouds, '%06d.ply' % point_cloud.frame)))
